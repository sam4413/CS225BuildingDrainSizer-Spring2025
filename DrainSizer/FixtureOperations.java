...
public class FixtureOperations {
    ...
    public void addFixture(Scanner input) {
        ...
        for (Fixture fixture : fixtures.getFixturesList()) {
            if (selectedDef.getFixtureName().equalsIgnoreCase("Floor drains") && fixture instanceof FloorDrain && fixture.getIsPublic() == isPublic) {
        ...
        if (!updatedExisting) {
            ...
            if (selectedDef.getFixtureName().equalsIgnoreCase("Floor drains")) {
                newFixture = new FloorDrain(qty, isPublic);
            } else if (selectedDef.getFixtureName().equalsIgnoreCase("Water closet (1.6 gpf)")) {
                newFixture = new WaterCloset(qty, isPublic);
            } else if (selectedDef.getFixtureName().equalsIgnoreCase("Kitchen sink")) {
                newFixture = new KitchenSink(qty, isPublic);
            } else if (selectedDef.getFixtureName().equalsIgnoreCase("Lavatory")) {
                newFixture = new Lavatory(qty, isPublic);
            }
    ...
    public void removeFixture(Scanner input) {
        ...
        String fixtureToRemove = InputHelper.processStringChoice(input, "Enter the name of the fixture to remove: ").trim();
        ...
        if (removed > 0) {
            Log.out(Color.GREEN + "Successfully removed " + removed + " fixture(s) named '" + fixtureToRemove + "'." + Color.RESET);
        } else {
